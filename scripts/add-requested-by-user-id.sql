...
CREATE INDEX IF NOT EXISTS idx_requests_requested_by_user_id ON public.requests(requested_by_user_id);
...
DROP POLICY IF EXISTS "Authorized users can manage requests" ON public.requests;
...
DROP POLICY IF EXISTS "Allow authenticated users to read all requests" ON public.requests;
DROP POLICY IF EXISTS "Allow authenticated users to update their own requests" ON public.requests;
DROP POLICY IF EXISTS "Allow authenticated users to delete their own requests" ON public.requests;
...
CREATE POLICY "Users can view requests" ON public.requests FOR SELECT USING (true);

-- Policy for users to create requests, ensuring requested_by_user_id matches their auth.uid()
CREATE POLICY "Users can create requests linked to their ID" ON public.requests FOR INSERT WITH CHECK (
...
CREATE POLICY "Authorized users can manage requests" ON public.requests FOR ALL USING (
    EXISTS (
        SELECT 1 FROM public.users
        WHERE id = auth.uid()
        AND (role = 'admin' OR role = 'inventory_manager') -- Example: check for admin or inventory_manager role
    )
);
